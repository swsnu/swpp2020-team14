...
describe('Signup', () => {
  it('should handle changes', async () => {
    ...
    while (comp.instance().state.email != 'TEST_EMAIL') await new Promise((r) => setTimeout(r, 100));
    while (comp.instance().state.password != 'TEST_PASSWORD') await new Promise((r) => setTimeout(r, 100));
    while (comp.instance().state.nickname != 'TEST_NICKNAME') await new Promise((r) => setTimeout(r, 100));
  });
